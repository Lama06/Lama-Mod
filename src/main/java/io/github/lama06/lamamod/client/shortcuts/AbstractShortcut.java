package io.github.lama06.lamamod.client.shortcuts;
 ...
 * zum Beispiel ein Command ausgeführt wird. Muss von jedem Shortcut erweitert werden.
 *
...
public abstract class AbstractShortcut<T extends ShortcutOptions> implements MessageSentCallback {
     ...
     */
    protected abstract T getShortcutOptions(ClientOptions options);

    protected T getShortcutOptions() {
        return getShortcutOptions(ClientOptions.getOptions());
    }
        ...
        if (msg.getText().startsWith(getName().toLowerCase(Locale.ROOT))) {
            ...
            if (args.length == 0) {
                if (options.enabled) {
                    ...
                    ClientUtil.sendMsgToPlayer("Der Shortcut ist jetzt deaktiviert");
                } else {
                    options.enabled = true;
                    ClientUtil.sendMsgToPlayer("Der Shortcut ist jeztz aktiviert");
                }
